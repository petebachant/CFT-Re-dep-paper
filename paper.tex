...
Colovos Rd., \mbox{Durham, NH \hl{zip code}, USA}}

% Contact information of the corresponding author (Add [2] after \corres if
% there are more than one corresponding author.)
\corres[2]{\hspace{-1em} Correspondence: pxL3@unh.edu (P.B.); martin.wosnik@unh.edu (M.W.); \linebreak Tel.: \hl{(P.B.); (M.W.)}}
...
\abstract{Experiments were performed with a large laboratory-scale high solidity
 ...
 estimator for predicting the Reynolds number dependence of an actual cross-flow
...
%please check the symbol ``\equiv '' and make sure it is correct
...
\textls[-10]{Static foil performance does not tell the whole story for a cross-flow turbine.
The azimuthal, and} therefore, temporal, variation of $\alpha$ in a cross-flow
turbine implies dynamic loading, encountering \textls[-10]{dynamic stall for tip speed ratios
...
Chamorro \emph{et al.} \cite{Chamorro2012} showed that turbulence statistics in an AFT%please define
...
similar to the Sandia National Labs/U.S. Department of Energy RM2%please define
 River Turbine
...
%please define NACA and add the information of Manufacturer, City, Country
  with a 0.14 m
...
not yet agreed upon for \hl {CFTs} %please define
...
Sentran \hl {ZB3 500-lbf} %please define/check the unit
...
\begin{figure}[H]
 ...
 %please mark (a) and (b) in the figure and label (a) and (b) in the figure caption.
...
%is it better to mark different color to the lines?
\label{fig:cp-curves}
\end{figure}
\unskip
...
%please use (a) and (b) to instead left and right and mark (a) and (b) in the figure.
...
number, \texttt{NCrit = 9} %please check the use of \texttt here
...
%please mark (a) and (b), (c) in the figure and label them in the caption
...
\caption{\hl {Reynolds} number dependence of the normalized peak torque coefficient
 ...
 %please pay attention to the value ``1e5'' in the figure. is it ``\times 10^5''. If yes, please revise it to \times 10 ^5 in the figure. meanwhile, is it better to make different color with the lines?
...
\authorcontributions{\textbf{Author Contributions:} \hl{For research articles with several authors, a short paragraph specifying their individual contributions must be provided.}}
