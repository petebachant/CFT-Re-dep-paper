...
model is not necessarily the same as its dynamical scale. In the case of
geometrically similar physical models of turbines and propellers operating under
kinematic similarity, i.e. at the same tip speed or advance ratio, however, the
dynamical scale can be (is) the same as the geometric scale. It is the dynamical
scale of the model which we are concerned with hereafter.


\todo[inline]{Need to find a good way to succintly introduce scaling 
subleties.}
...
Full power and rotor drag coefficient curves for various Reynolds numbers are
...
These same wake maps were taken for $Re_D = 0.4 \times 10^6$, $0.6 \times 10^6$,
...
airfoil performance, e.g., lift-to-drag ratio. Is it also shown from this metric
